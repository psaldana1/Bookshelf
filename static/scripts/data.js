...
function showContent(data) {
    ...
    if (data.items.length > 0)                                                    //check to see if response object contains any response aka books 
    {
        ...
        data.items.forEach(function (element) {                                                    //for every item (book) in response object (for loop)
            ...
            clon.getElementById("authors").innerHTML = `${element.volumeInfo.authors[0]}`;
...
function addTo(data) {
    let divContainer = document.getElementById("addTo");

    // If we have time we need to create our own non-SQL database with mongodb & mongoose
